...
```{r berths_plot, echo=F}
mapview(st_transform(berths,4326), map.types = c("OpenStreetMap.BlackAndWhite")) %>% addFeatures(st_transform(river,4326))
```

### Map of 1km emissions (sailing PM2.5 shown as example)

```{r grid_emissions, include=F}
...
```{r emissions_pm25_plot, echo=F}
mapview(st_transform(grid_emissions[grid_emissions$Substance == 'PM2.5','Sailing_kg'],4326), map.types = c("OpenStreetMap.BlackAndWhite")) %>% addFeatures(st_transform(river,4326))
```
...
```{r turn_into_raster_stack, include=F}
#Empty raster
r  <- raster(as(grid_emissions, "Spatial"),  res = 1000)

pollutants <- unique(grid_emissions$Substance)

for (i in 1:length(pollutants)) {
  
  berth          <- rasterize(as(grid_emissions[grid_emissions$Substance == pollutants[i],], "Spatial"), r, field = 'AtBerth_kg')
  names(berth)   <- paste0('berth_', pollutants[i])
  sailing        <- rasterize(as(grid_emissions[grid_emissions$Substance == pollutants[i],], "Spatial"), r, field = 'Sailing_kg')
...
```{r emissions_sailing_co_raster_plot, echo=F}
river  <- st_read('X:/Projects/PLA/GIS/Shapefiles/River Thames Outline.shp') %>% st_set_crs(27700)

plot <- levelplot(emissions[['sailing_N2O']],
          margin=F,
          col.regions=viridis,
          scales=list(draw=F)) + 
...
```{r emissions_berth_co_raster_plot, echo=F}
river  <- st_read('X:/Projects/PLA/GIS/Shapefiles/River Thames Outline.shp') %>% st_set_crs(27700)

plot <- levelplot(emissions[['berth_N2O']],
          margin=F,
          col.regions=viridis,
          scales=list(draw=F)) + 
  layer(sp.polygons(as(river, 'Spatial'), col ='red'))
...
```{r make_10m_grid, echo=FALSE, warning=FALSE, include=F}
...
```{r import_ais_data, echo=F, warning=FALSE, include=F}
...
```{r sailing_gps_points, echo=F}
river  <- st_read('X:/Projects/PLA/GIS/Shapefiles/River Thames Outline.shp') %>% st_set_crs(27700)

plot <- levelplot(sailing_grid[['count']],
          margin=F,
          col.regions=viridis,
          scales=list(draw=F),
...
```{r berth_gps_points, echo=F}
river  <- st_read('X:/Projects/PLA/GIS/Shapefiles/River Thames Outline.shp') %>% st_set_crs(27700)

plot <- levelplot(berth_grid[['count']],
          margin=F,
          col.regions=viridis,
          scales=list(draw=F),
          xlim=c(535000,545000),
...
```{r aggregate_up_to_100m, include=F}
...
```{r disagreggate_down_to_10m, include=F}
...
```{r results_pm25_sailing_plot}
river  <- st_read('X:/Projects/PLA/GIS/Shapefiles/River Thames Outline.shp') %>% st_set_crs(27700)

plot <- levelplot(pm25_sailing_emissions,
          margin=F,
          col.regions=viridis,
          scales=list(draw=F),
          xlim=c(535000,545000),
...
```{r results_pm25_berth_plot}
river  <- st_read('X:/Projects/PLA/GIS/Shapefiles/River Thames Outline.shp') %>% st_set_crs(27700)

plot <- levelplot(pm25_berth_emissions,
          margin=F,
          col.regions=viridis,
          scales=list(draw=F),
          xlim=c(535000,545000),
